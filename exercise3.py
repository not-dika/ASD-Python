...
gk = gp*1.2 if status == "Y" else gp if status == "N" else 0
gb = gk*0.9
print("="*15+f"\nNama: {nama}\nJabatan: {jabatan}\nStatus Kawin: {status}\nGaji Kotor: Rp. {gk:,}\nPajak: Rp. {(gk-gb):,}\nTotal Pendapatan: Rp. {gb:,}" if gb > 0 else "Data jabatan yang anda masukan invalid" if gk == 0 else "Data kawin yang anda masukan invalid")